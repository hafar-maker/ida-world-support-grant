...
export default function MyApplicationsPage() {
  ...
  async function search(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/applications/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) })
  ...
  return <><SiteHeader/><main className="min-h-screen bg-[#F4F7F9] px-4 py-10"><div className="mx-auto max-w-5xl"><div className="rounded-xl border bg-white p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-wider text-[#005EA8]">Applicants</p><h1 className="mt-2 text-2xl font-extrabold text-[#12304A]">My Applications</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Enter the email address used on your application to see your submission status. There is no OTP and no password.</p><form onSubmit={search} className="mt-6 flex flex-col gap-3 sm:flex-row"><div className="flex flex-1 items-center gap-2 rounded-lg border border-slate-300 px-3"><Mail size={17} className="text-slate-400"/><input required type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" className="w-full border-0 px-1 py-3 text-sm outline-none"/></div><button disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#005EA8] px-5 py-3 text-sm font-bold text-white disabled:opacity-50"><Search size={16}/>{loading?'Searching…':'Track applications'}</button></form>{error&&<div className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div>}
