...
export function ChatPanel({ applicationId, applicantName, staff = false }: { applicationId?: string; applicantName?: string; staff?: boolean }) {
  ...
  async function enableNotifications() {
    ...
    try {
      ...
      if (permission === 'granted') {
        new Notification('Live chat notifications enabled', {
          body: 'You will be notified when a new chat message arrives.',
        })
      }
    } catch {
      setError('Your browser could not enable notifications.')
    }
  }
  ...
  function notifyIncomingMessage(message: ChatMessage) {
    ...
    new Notification(staff ? `New message from ${applicantName || 'applicant'}` : 'New message from the grant team', {
  ...
  const notificationButton = notificationPermission === 'granted'
    ? <span className="ml-auto inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-600" title="Chat notifications are enabled"><Bell size={13}/> Notifications on</span>
