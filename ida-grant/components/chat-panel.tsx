...
export function ChatPanel({ applicationId, applicantName, staff = false }: { applicationId?: string; applicantName?: string; staff?: boolean }) {
  ...
  function notifyIncomingMessage(message: ChatMessage) {
    ...
    new Notification(staff ? `New message from ${applicantName || 'applicant'}` : 'New message from the grant support team', { body: message.message, tag: `chat-${threadId}` })
  }
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex items-center gap-3 border-b bg-white px-5 py-4">
      ...
      <div><h2 className="text-sm font-extrabold text-[#12304A]">{headerTitle}</h2><p className="text-xs text-slate-500">{headerSubtitle}</p></div>
